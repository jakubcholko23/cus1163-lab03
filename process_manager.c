...
int run_basic_demo(void) {
   ...
   if(pipe(pipe_fd) == -1){
      perror("There is a problem while using pipe");
      return -1;
   }
   ...
   if(producer_pid == 0){
     ...
     perror("Fork failed for Producer Child");
     return -1;
   }
  ...
  if(consumer_pid == 0){
       ...
       perror("Fork Failed for Consumer Child");
  }
 ...
 child_pid = waitpid(consumer_pid,&status,1);
 printf("Child %d exited with status %d\n",child_pid,status);
...
int run_multiple_pairs(int num_pairs) {
   ...
   for(int i = 0; i<num_pairs; i++){
      int pipe_fd[2];
      if(pipe(pipe_fd) == -1){
            perror("There is a problem using pipe");
            return -1;
      }
      printf("\n==== Pair %d ====\n", i + 1);
     ...
     if(consumer_pid == 0){
          ...
          exit(0);
            ...
            perror("Fork Failed for consumer chil");
            return -1;
     }
     close(pipe_fd[0]);
     close(pipe_fd[1]);
    ...
    for(int i = 0; i< pid_count;i++){
        pid_t child_pit = waitpid(pids[i],&status,0);
        if(child_pit == -1){
           perror("Error in waitpid");
        }
        else{
           printf("Child (PID: %d) finished with the status %d\n",child_pit,status);
        }
    }
